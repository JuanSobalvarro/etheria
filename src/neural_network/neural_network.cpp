...
NeuralNetwork::NeuralNetwork(const NeuralNetworkConfig& cfg): config(cfg) {
    ...
    for (size_t l = 0; l + 1 < config.layer_sizes.size(); ++l) {
        ...
        double stddev;
        act::ActivationFunctionType act = (l + 1 == config.layer_sizes.size() - 1) ? config.output_activation : config.hidden_activation;
        if (act == act::RELU || act == act::SOFTPLUS) {
            stddev = std::sqrt(2.0 / in); // He
        } else if (act == act::SIGMOID || act == act::TANH) {
...
double NeuralNetwork::activation(double x, act::ActivationFunctionType type) {
    return act::forward(type, x);
}

double NeuralNetwork::activation_derivative(double x, act::ActivationFunctionType type) {
    return act::derivative(type, x);
}

Vector NeuralNetwork::applyActivation(const Vector& z, act::ActivationFunctionType type) {
    Vector a(z.size());
    for (size_t i = 0; i < z.size(); ++i) a[i] = activation(z[i], type);
    return a;
}

Vector NeuralNetwork::applyActivationDerivative(const Vector& z, act::ActivationFunctionType type) {
    Vector d(z.size());
...
Vector NeuralNetwork::predict(const Vector& input) const {
    ...
    for (size_t l = 0; l < weights.size(); ++l) {
        act::ActivationFunctionType act = (l == weights.size()-1) ? config.output_activation : config.hidden_activation;
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        for (size_t sample = 0; sample < inputs.size(); ++sample) {
            ...
            // Output delta
            {
                ...
                act::ActivationFunctionType act = config.output_activation;
            ...
            for (int l = static_cast<int>(weights.size()) - 2; l >= 0; --l) {
                act::ActivationFunctionType act = (l == static_cast<int>(weights.size()) - 1) ? config.output_activation : config.hidden_activation;
                act::ActivationFunctionType act_hidden = (l == static_cast<int>(weights.size()) - 1) ? config.output_activation : config.hidden_activation;
